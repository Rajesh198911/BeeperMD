...
public class BookAppointments extends Base {
	...
	public void scheduleYourTestNew(String coupon, String RequestedTestingAppointmentDate, String Time,
		...
		bookAppointmentStep1(coupon, RequestedTestingAppointmentDate, Time, SelectRecurring, RequestedTestingAddress,
				City, State, ZipCode, Inbuilding, FirstName, LastName, Gender, DOB, CellPhone, Email,
				ReceiveNotificationsFromUs);

		bookAppointmentStep2(TotalAdditionalMembers, MembersInfo);

		bookAppointmentStep3(TypeOfCovidTest);

	}

	// Book Appointment Step 1
	public void bookAppointmentStep1(String coupon, String RequestedTestingAppointmentDate, String Time,
			String SelectRecurring, String RequestedTestingAddress, String City, String State, String ZipCode,
			String Inbuilding, String FirstName, String LastName, String Gender, String DOB, String CellPhone,
			String Email, String ReceiveNotificationsFromUs) throws Exception {

		// Click on schedule your test button
		VisibilityOfElementByXpath(
				"//p[contains(text(),'*For instant results, Antigen and Antibody tests are')]/following-sibling::a", 10)
						.click();

		// Validate sign-up form open
		Assert.assertTrue(
				VisibilityOfElementByXpath("//h3[starts-with(normalize-space(text()),'Appointment Information')]", 10)
						.isDisplayed());
		...
		VisibilityOfElementByXpath("//input[@id='actualScheduledDate']", 10).sendKeys(RequestedTestingAppointmentDate);
		Thread.sleep(1000);
		...
		if (Inbuilding.equalsIgnoreCase("yes")) {
			VisibilityOfElementByXpath("//input[@id='parkingFeeEnabled']/following-sibling::label/span", 10).click();
			Thread.sleep(seconds);
		}
		...
		// Validate text
		Assert.assertTrue(VisibilityOfElementByXpath("//div[contains(text(),'You need to pay')]", 10).isDisplayed());
		Thread.sleep(seconds);
	}

	// Book Appointment Step 2
	public void bookAppointmentStep2(String TotalAdditionalMembers, String MembersInfo) throws Exception {
		...
		if (total_additional_members >= 1) {
			...
			for (int i = 0; i < total_additional_members; i++) {
				// Add Member Button
				VisibilityOfElementByXpath("//i[contains(text(),'(Adding a member will remove ALL the extra fees)')]",
						10).click();
				Thread.sleep(50);
			}
			...
			for (String myStr : splits) {
				...
				if (myStr.contains("Patient Information")) {
					...
					if (myStr.contains("fname")) {
						first_name.get(s).sendKeys(myStr.split("=")[1].trim());
					}

					else if (myStr.contains("lname")) {
						last_name.get(s).sendKeys(myStr.split("=")[1].trim());
					}

					else if (myStr.contains("gender")) {
						gen.get(s).sendKeys(myStr.split("=")[1].trim());

					}

					else if (myStr.contains("dob")) {
						birth.get(s).sendKeys(myStr.split("=")[1].trim());
						Thread.sleep(50);
						...
						ph.get(s).clear();
						Thread.sleep(seconds);
						TypeInFieldSlowly(ph.get(s), myStr.split("=")[1].trim());
						...
						mail.get(s).clear();
						Thread.sleep(seconds);
						mail.get(s).sendKeys(myStr.split("=")[1].trim());
						s = s + 1;
					}
		...
		Assert.assertTrue(VisibilityOfElementByXpath("//h3[contains(text(),'Appointment Type')]", 10).isDisplayed());
		Thread.sleep(seconds);
	}

	// Book Appointment Step 3
	public void bookAppointmentStep3(String TypeOfCovidTest) throws Exception {
		...
		for (int k = 0; k < type_of_covid_test.size(); k++) {
			...
			if (type_of_covid_test.get(k).contains("Covid-19 PCR Screening")) {
				VisibilityOfElementByXpath("//div[contains(text(),\"" + type_of_covid_test.get(k) + "\")]", 10).click();
				Thread.sleep(200);
			}
		...
		Assert.assertTrue(
				VisibilityOfElementByXpath("//span[contains(text(),'We have sent an email with a link to upload')]", 30)
						.isDisplayed());
